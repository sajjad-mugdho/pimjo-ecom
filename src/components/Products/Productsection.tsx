...
const sampleProducts: Product[] = [
  ...
  // duplicate some to show pagination
  {
    id: 6,
...
const StatusBadge = ({ status }: { status: Product["status"] }) => {
  if (status === "Delivered") {
    return (
      <div className="w-[70px] h-[22px] flex items-center justify-center" aria-hidden>
        <svg
          width="70"
          height="22"
          viewBox="0 0 70 22"
...
const Productsection = () => {
  ...
  const filtered = useMemo(() => {
    const byQuery = sampleProducts.filter((p) => p.product.toLowerCase().includes(query.toLowerCase()));
    return selectedStatuses.length > 0 ? byQuery.filter((p) => selectedStatuses.includes(p.status)) : byQuery;
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8">
      <section className="max-w-full bg-white rounded-2xl shadow-md py-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 px-4 py-3">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="relative w-full sm:w-[320px]">
              <input
                ...
                className="w-full pl-10 pr-3 py-2 rounded-[8px] border border-[#D0D5DD] text-sm text-gray-700 bg-white focus:outline-none"
              ...
              <svg
                ...
                aria-hidden
              >
                <path d="M21 21l-4.35-4.35" stroke="#98A2B3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                <circle cx="11" cy="11" r="6" stroke="#98A2B3" strokeWidth="1.5" />
              </svg>
            </div>

            <Popover>
              <PopoverTrigger asChild>
                <button className="inline-flex items-center h-10 gap-2 px-3 py-1 rounded-[8px] border border-[#D0D5DD] text-sm text-gray-700 hover:bg-gray-50">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M3 5h18M8 12h8M10 19h4" stroke="#344054" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                  Filter
                </button>
              </PopoverTrigger>

              <PopoverContent side="bottom" align="start" sideOffset={8} className="p-3 bg-white rounded-md shadow-lg border">
                <div className="flex flex-col gap-2">
                  <label className="inline-flex items-center gap-2">
                    <input type="checkbox" checked={selectedStatuses.includes("Delivered")} onChange={() => toggleStatus("Delivered")} className="w-4 h-4" />
                    <span className="text-sm">Delivered</span>
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input type="checkbox" checked={selectedStatuses.includes("Pending")} onChange={() => toggleStatus("Pending")} className="w-4 h-4" />
                    <span className="text-sm">Pending</span>
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input type="checkbox" checked={selectedStatuses.includes("Cancelled")} onChange={() => toggleStatus("Cancelled")} className="w-4 h-4" />
                    <span className="text-sm">Cancelled</span>
                  </label>
                  <div className="pt-2 flex justify-end">
                    <button onClick={() => setSelectedStatuses([])} className="text-sm text-[#667085] px-2 py-1 rounded hover:bg-gray-50">Clear</button>
                  </div>
                </div>
              </PopoverContent>
            </Popover>
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="min-w-[180px] text-xs text-[#667085] font-medium">Product</TableHead>
                <TableHead className="text-xs text-[#667085] font-medium">Category</TableHead>
                <TableHead className="text-xs text-[#667085] font-medium">Price</TableHead>
                <TableHead className="text-xs text-[#667085] font-medium">Status</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {pageData.map((p) => (
                <TableRow key={p.id}>
                  <TableCell className="font-medium flex items-center gap-3">
                    <Image src={p.image} alt={p.product} width={50} height={50} />
                    <div>
                      {p.product}
                      <p className="text-xs font-normal leading-6 text-gray-500">2 Variants</p>
                    </div>
                  </TableCell>
                  <TableCell className="text-sm text-[#667085]">{p.category}</TableCell>
                  <TableCell className="text-sm text-[#667085]">{p.price}</TableCell>
                  <TableCell>
        ...
        <div className="px-4 py-3 flex items-center justify-between border-t border-gray-200">
          <div className="text-sm text-gray-600">Showing {(page - 1) * pageSize + 1} - {Math.min(page * pageSize, filtered.length)} of {filtered.length} products</div>
            ...
            <div className="hidden sm:flex items-center gap-1">
              {Array.from({ length: totalPages }).map((_, i) => (
                <button
                  key={i}
                  onClick={() => setPage(i + 1)}
                  className={`w-8 h-8 inline-flex items-center justify-center rounded-md border ${page === i + 1 ? "bg-[#F2F4F7]" : "bg-white"}`}
                >
                  {i + 1}
                </button>
              ))}
