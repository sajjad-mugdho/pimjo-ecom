...
export default function DashboardHeader() {
  ...
  const setDark = useDashboardStore((s) => s.setDark);
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("pimjo-theme");
      if (stored === "dark") {
        setDark(true);
      } else if (stored === "light") {
        setDark(false);
      } else {
        // prefer OS preference
        const prefersDark =
  ...
  useEffect(() => {
    try {
      ...
      if (dark) {
        ...
        localStorage.setItem("pimjo-theme", "dark");
      } else {
        root.classList.remove("dark");
        localStorage.setItem("pimjo-theme", "light");
      }
    } catch {
      // ignore
    }
  ...
  return (
    <header className="h-16 flex items-center justify-between px-2 md:px-6 py-4 border-b border-gray-200 dark:border-[#1D2939] bg-white dark:bg-[#101828] relative z-50">
      <div className="flex items-center gap-3">
        <button
          ...
          className={`inline-flex items-center justify-center h-11 w-11 rounded-[8px] border border-gray-200 dark:border-[#1D2939] bg-white dark:bg-[#101828] hover:bg-gray-50`}
        >
          {mobileOpen ? (
        ...
        <div className="hidden md:block mx-4" style={{ width: 430 }}>
          <div
            className="h-[44px] flex items-center justify-between border-[#e4e7ec] dark:border-[#1D2939] rounded-[8px] border"
            style={{
              borderWidth: 1,
      ...
      <div className="hidden ml-auto md:flex items-center gap-4">
        ...
        <button
          ...
          aria-pressed={dark}
          onClick={() => toggleDark()}
          className="inline-flex items-center justify-center h-10 w-10 rounded-full border border-gray-200 dark:border-[#1D2939] bg-white dark:bg-[#101828] hover:bg-gray-50 hover:dark:bg-[#1D2939]"
          title={dark ? "Switch to light mode" : "Switch to dark mode"}
          style={{ color: "#667085" }}
        >
          {dark ? (
        ...
        <div className="relative">
          <div className="relative">
            <button
              ...
              aria-label="Notifications"
              onClick={() => setNotifOpen((v) => !v)}
              className="inline-flex items-center justify-center h-10 w-10 rounded-full border border-gray-200 dark:border-[#1D2939] bg-white dark:bg-[#101828] hover:bg-gray-50 relative"
              style={{ color: "#667085" }}
            >
              <svg
                width="18"
        ...
        <div className="relative">
          <div className="relative inline-block">
            <button
              onClick={() => setProfileOpen((v) => !v)}
              className="flex items-center gap-2 px-2 py-1 rounded-md hover:bg-gray-50 dark:hover:bg-[#1D2939] cursor-pointer"
              aria-haspopup="true"
              aria-expanded={profileOpen}
            >
              <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center text-sm font-medium text-[#667085] dark:text-[#98A2B3]">
                <Image
                  src="/avatar-1.png"
                  alt="User Avatar"
                  width={40}
              ...
              <div className="flex flex-col min-w-20 leading-none">
                <span className="text-sm font-medium text-[#344054] dark:text-[#98A2B3] leading-5">
                  Sajjad Hossen
                </span>
              </div>
              <div>
